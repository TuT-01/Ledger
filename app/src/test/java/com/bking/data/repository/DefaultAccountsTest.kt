...
class DefaultAccountsTest {
    ...
    fun `default accounts include other categories for income and expense`() {
        ...
        assertEquals("其他收入", accountsById.getValue(IncomeKind.OTHER.accountId).name)
        assertEquals(AccountGroup.EXPENSE, accountsById.getValue(ExpenseKind.OTHER.accountId).group)
        assertEquals("其他支出", accountsById.getValue(ExpenseKind.OTHER.accountId).name)
    }

    @Test
    fun `income and expense kinds expose low key other category`() {
        assertTrue(IncomeKind.entries.any { it.label == "其他收入" && it.icon == "..." })
        assertTrue(ExpenseKind.entries.any { it.label == "其他支出" && it.icon == "..." })
    }
}
